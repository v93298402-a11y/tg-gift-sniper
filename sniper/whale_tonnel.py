...
enriched via :func:`sniper.whale_enrich.enrich_attributes` before
...
async def run_tonnel_feed(
    ...
    async def _on_new_post(event):  # noqa: ANN001
        ...
        try:
            ...
            logger.exception("Tonnel: parser crashed on message %s", event.id)
            return
        if sale is None:
            return
        ...
        # Enrich attributes — Tonnel posts never include them.
